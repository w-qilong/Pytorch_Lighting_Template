...
class ExampleEvaldata(Dataset):
    def __init__(self):
        ...
        self.data_file = 't10k-images-idx3-ubyte'
        self.label_file = 't10k-labels-idx1-ubyte'
        self.eval_data, self.eval_labels = load_data(self.folder, self.data_file, self.label_file)
        self.transforms = transforms

    def __getitem__(self, index):
        img, target = self.eval_data[index], int(self.eval_labels[index])
...
if __name__ == '__main__':
    ...
    minist_eval_dataset=ExampleEvaldata()
    print(len(minist_eval_dataset))

    dataloader = DataLoader(dataset=minist_eval_dataset, batch_size=2, shuffle=False, num_workers=4)
